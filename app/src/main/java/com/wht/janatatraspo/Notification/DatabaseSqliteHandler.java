...
public class DatabaseSqliteHandler extends SQLiteOpenHelper {
    ...
    private static final String KEY_NOTIFICATION_LINK = "notification_link";
    ...
    public void onCreate(SQLiteDatabase db) {
        String CREATE_NOTIFICATION_TABLE = "CREATE TABLE IF NOT EXISTS " + TABLE_NOTIFICATION + "("
                + KEY_NOTIFICATION_ID + " INTEGER PRIMARY KEY,"
                + KEY_NOTIFICATION_TITLE + " TEXT,"
                + KEY_NOTIFICATION_MESSAGE + " TEXT,"
                + KEY_NOTIFICATION_IMAGE + " TEXT,"
                + KEY_NOTIFICATION_DATE_TIME + " TIMESTAMP DEFAULT (datetime('now','localtime')) NOT NULL,"
                + KEY_NOTIFICATION_READ + " INTEGER,"
                + KEY_NOTIFICATION_LINK + " TEXT "
                + ")";
    ...
    public void insert_notification(String title, String message, String image, String link) {
        ...
        values.put(KEY_NOTIFICATION_LINK, link);
        Log.d("value", "insert_notification: "+values);
        // Inserting Row
    ...
    public long getNotificationCount1() {
        String countQuery = "SELECT * FROM " + TABLE_NOTIFICATION +" WHERE "+KEY_NOTIFICATION_READ+" == '0'";
      //  String countQuery = "SELECT * FROM " + TABLE_NOTIFICATION;
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery(countQuery, null);
    ...
    public void UpdateNotification() {
        SQLiteDatabase db = this.getReadableDatabase();
        db.execSQL("UPDATE " + TABLE_NOTIFICATION + " SET " + KEY_NOTIFICATION_READ + "='1'");
        db.close();
    }
