...
public class Validations {
    ...
    public boolean isTextviewBlank(TextView string) {
        return string.getText().toString().trim().equals("");
    }
...
//    }
    ...
    public boolean isPanCard(EditText editText) {
        return Pattern.compile("/^([a-zA-Z]([a-zA-Z]([a-zA-Z]([a-zA-Z]([a-zA-Z]([0-9]([0-9]([0-9]([0-9]([a-zA-Z])?)?)?)?)?)?)?)?)?)?$/").matcher(editText.getText().toString().trim()).matches();
    }

    public boolean isaadharCard(EditText editText) {
        return Pattern.compile("^[2-9]{1}[0-9]{3}\\\\s[0-9]{4}\\\\s[0-9]{4}$").matcher(editText.getText().toString().trim()).matches();
    }
