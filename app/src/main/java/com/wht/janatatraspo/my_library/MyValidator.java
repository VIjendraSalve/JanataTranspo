...
public class MyValidator {
    ...
    public static boolean isValidName(EditText editText) {
        String email = editText.getText().toString().trim();
        Pattern pattern = Pattern.compile(VEHICLE_REGEX2);
        Matcher matcher = pattern.matcher(email);
        if (!matcher.matches()) {
    ...
    public static boolean isValidField1(TextView editText) {
...
*/
    ...
    public static boolean isMatchPassword(EditText etNewPasword, EditText etConfirmNewPasword) {
        ...
        if (!isValidPassword(etNewPasword)) {
            ...
            etConfirmNewPasword.setError("Password doesn't match");
            etConfirmNewPasword.requestFocus();
            return false;
        } else {
